// 1. Output user identity information, including IP address, MAC address, and NCC student ID
console.log("I am Jason. my IP is 10.50.43.81 Mac address is B4-45-06-B2-77-F1. Ncc student ID is:223190719");
// 2. Import and initialize the SQLite3 module to enable the detailed log mode
const sqlite3 = require('sqlite3').verbose();

// 3. Connect to the local book.db database file. If the connection is successful, a message is displayed. Otherwise, print an error message
const db = new sqlite3.Database("./book.db", (err) => {
    if (err) {
        return console.error(err.message);
    }
    console.log('Connected to the books database.');
});


// 4. Create a table named "books" if it does not exist. The table contains: ID (primary key), title, author, ISBN, and description fields. After the creation, a message is displayed
const readline = require('readline').createInterface({
    input: process.stdin,
    output: process.stdout
});


// create the table if not exists
db.run('CREATE TABLE IF NOT EXISTS books (ID INTEGER PRIMARY KEY, title TEXT, author TEXT, ISBN TEXT, description TEXT)', (err) => {
    if (err) {
        return console.error(err.message);
    }
    console.log('Books Table created successfully');

    commandInterface();
});

function listBooks() {
    // list all books
    db.all('SELECT * FROM books', (err, rows) => {
        if (err) {
            return console.error(err.message);
        }
        rows.forEach((row) => {
            console.log(`ID: ${row.ID}, Title: ${row.title}, Author: ${row.author}, ISBN: ${row.ISBN}, Description: ${row.description}`);
        });
    });
}
// queue -> task 1(callback)
function commandInterface() {
    readline.question('Enter book title: ', (title) => {
        readline.question('Enter book author: ', (author) => {
            readline.question('Enter book ISBN: ', (ISBN) => {
                readline.question('Enter book description: ', (description) => {
                    // insert the book into the database
                    db.run('INSERT INTO books (title, author, ISBN, description) VALUES (?, ?, ?, ?)', [title, author, ISBN, description], (err) => {
                        if (err) {
                            return console.error(err.message);
                        }
                        console.log('Book added successfully.');
                        readline.question('Do you want to add another book? (yes/no): ', (answer) => {
                            if (answer === 'yes') {
                                commandInterface();
                            } else {
                                listBooks();
                                
                                readline.close();
                            }
                        });
                    });
                });
            });
        });
    });
    // callback hell
}